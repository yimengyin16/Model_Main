...
p <- ggplot(data=tmp %>% filter(year<=30), aes(x=year, y=ercprdiffgt10, colour=runname2lines)) +
  ...
  theme(legend.direction = "vertical", 
        ...
        panel.grid.minor.x = element_blank(),
        panel.grid.minor.y = element_blank(),
        panel.grid.major.y = element_line(size = 0.5, color = "gray80")) + RIG.theme()
...
roi.labs
roi.labs.2lines[5] <- "SOA Blue Ribbon \n15-year\nopen percent asset-5\nDiscout 5.9%"
...
p <- ggplot(data=tmp %>% filter(year<=30), aes(x=year, y=ercprdiffgt10, colour=runname2lines)) +
  ...
  guides(colour=guide_legend(title="Funding scenario:"), size=guide_legend(title=NULL)) + RIG.theme()

p

outputs.dir <- "IO_M1_new/MFC2017/"

ggsave(paste0(outputs.dir,"fig8_ercprdiffgt10.png"), p, width=10*0.8, height=6*0.8, units="in")
...
p
ggsave(paste0(outputs.dir,"fig10_fr7p5lt40.png"), p, width=10*0.8, height=6*0.8, units="in")
