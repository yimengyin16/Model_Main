...
load(paste0(runsd,"Outputs_A1F075_O30pA5.RData"))

g.distFR <- outputs_list$results %>% filter(year == 30, sim > 0) %>% 
  ggplot(aes(FR_MA)) + theme_bw() + 
  geom_histogram( fill = RIG.blue, binwidth = 5, boundary = 0)+
  coord_cartesian(xlim = c(0,250))  

ggsave(file = paste0(outputs.dir, "DistFR.png"), g.distFR, height = 5.5, width = 9)
