...
# Check steady state
d.SS <- pop$active %>% filter(year == 100, age <= 74) %>% select(-year)
d.SS2 <- pop$active %>% filter(year == 101, age <= 74) %>% rename(number.a2 = number.a) %>% select(-year)
...
d.newHires <- data.frame(age = 20:74, entrants_dist)
...
write.xlsx2(d.SS,file = "Data/Demo_Steady.xlsx", sheetName = "SS.actives", row.names = F )
write.xlsx2(d.SS_byAge,file = "Data/Demo_Steady.xlsx", sheetName = "SS.actives.ByAge", append = T, row.names = T)
write.xlsx2(d.SS_byYos,file = "Data/Demo_Steady.xlsx", sheetName = "SS.actives.ByYos", append = T, row.names = T)
write.xlsx2(d.newHires,file = "Data/Demo_Steady.xlsx", sheetName = "Dist_newHires", append = T, row.names = T)
write.xlsx2(d.SS.ret  ,file = "Data/Demo_Steady.xlsx", sheetName = "SS.retirees", append = T, row.names = T)
