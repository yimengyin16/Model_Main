...
df_age <- actives %>% filter(planname == planName, age < 60) %>% group_by(age) %>% 
          summarize(nactives_age = sum(nactives)) %>% mutate(pct_age =100* nactives_age / sum(nactives_age)) 

df_ea <-  actives %>% filter(planname == planName, age < 60) %>% group_by(ea)  %>% 
          summarize(nactives_ea = sum(nactives))  %>% mutate(pct_ea =100* nactives_ea / sum(nactives_ea)) %>% rename(age = ea)

df_yos <- actives %>% filter(planname == planName, age < 60) %>% group_by(yos) %>% summarize(nactives_yos = sum(nactives)) %>% mutate(pct_yos =100* nactives_yos / sum(nactives_yos))

df_dist <- data.frame(age = 20:59, pct_ent = 100*get_entrantsDist(actives, planName, simple = TRUE))
