...
Next stpes:
Need to do a cell-by-cell analysis to better understand the relationship between EAN and PUC, and their interaction
with salary scale.
...
# ADC rate(also NC rate) (no Entrants)

```{r, echo = FALSE, cache = F , eval = TRUE}

results_all %>% select(runname, year, ADC_PR) %>% filter(runname %in% paste0(c("EAN_", "PUC_"),"Avg_ne")) %>% 
  ggplot(aes(year, ADC_PR, color = runname)) + geom_line() + geom_point() + coord_cartesian(ylim = c(0,30))

results_all %>% select(runname, year, ADC_PR) %>% filter(runname %in% paste0(c("EAN_", "PUC_"),"Unif_ne")) %>% 
  ggplot(aes(year, ADC_PR, color = runname)) + geom_line() + geom_point() + coord_cartesian(ylim = c(0,30))

results_all %>% select(runname, year, ADC_PR) %>% filter(runname %in% paste0(c("EAN_", "PUC_"),"mc_ne")) %>% 
  ggplot(aes(year, ADC_PR, color = runname)) + geom_line() + geom_point() + coord_cartesian(ylim = c(0,30))
...
# Difference in ADC rate (no Entrants)
...
results_all %>% select(runname, year, ADC_PR) %>% spread(runname, ADC_PR) %>% 
  mutate(Diff_Avg_ne  = PUC_Avg_ne - EAN_Avg_ne,
         Diff_Unif_ne = PUC_Unif_ne - EAN_Unif_ne,
         Diff_mc_ne   = PUC_mc_ne -EAN_mc_ne ) %>% 
  select(year, starts_with("Diff")) %>% 
  gather(PopDist, Diff, -year) %>% 
  ggplot(aes(year, Diff, color = PopDist)) + geom_line() + geom_point() + coord_cartesian(ylim = c(0,20))
```


# %Difference in ADC (also in ADC rate) (no Entrants)

```{r, echo = FALSE}
results_all %>% select(runname, year, ADC_PR) %>% spread(runname, ADC_PR) %>% 
  mutate(DiffPct_Avg_ne  = 100*(PUC_Avg_ne - EAN_Avg_ne)/EAN_Avg_ne ,
         DiffPct_Unif_ne = 100*(PUC_Unif_ne - EAN_Unif_ne)/EAN_Unif_ne,
         DiffPct_mc_ne   = 100*(PUC_mc_ne - EAN_mc_ne)/EAN_mc_ne) %>% 
  select(year, starts_with("Diff")) %>% 
  gather(PopDist, DiffPct, -year) %>% 
  ggplot(aes(year, DiffPct, color = PopDist)) + geom_line() + geom_point() + coord_cartesian(ylim = c(0,1500))
