...
### Funded Ratio
```{r S4_FR, echo = FALSE, cache = F}

Runs <- c("Smoothed Policy", "Typical Policy")
...
## Examples of Single Simulations
...
demoSingle1 %>% filter(runname == "Typical Policy") %>% ggplot(aes(x = year, y = value, color = factor(sim) )) + theme_bw() + 
...
```{r S4_singleSim2, echo = FALSE, cache = F， eval = F}
...
```{r geoReturn, echo = FALSE, eval = FALSE}
